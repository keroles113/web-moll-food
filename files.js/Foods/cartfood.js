...
fetch(`https://login1form-3fad9-default-rtdb.firebaseio.com/CurrentUser.json`).then((res)=>{
    ...
    let arr = [];
    fetch(`https://login1form-3fad9-default-rtdb.firebaseio.com/user/${res.fullName}/products.json`)
    .then((res)=>{
        let myData = res.json();
        return myData;
    }).then((res)=>{
        ...
        if (arr.length == 0) {
            ...
            // head.style.cssText = `opacity: 0;`;
        }else{
            payment.style.cssText = `display: block`;
            head.style.cssText = `opacity: 1;`;
            total_Price.style.cssText = `display: block`
        }
        ...
        arrOrder.forEach(ele => {
            console.log(ele)
            priceSub.innerHTML = (parseFloat(priceSub.innerHTML) + parseFloat(ele.quantity * +ele.price)).toFixed(2)+'$';
            totalPric.innerHTML = (parseFloat(ships.innerHTML) + parseFloat(priceSub.innerHTML)).toFixed(2) + "$";
            let tr = document.createElement('tr');
    
            let td1 = document.createElement('td');
            let img = document.createElement('img');
            img.style.cssText = `width: 80%;`
            ...
            h5.appendChild(document.createTextNode(ele.name));
            td2.appendChild(h5);
            tr.appendChild(td2);
            ...
            btnminus.style.cssText = `width: 32px; height:32px; z-index: 1; 32px; border-radius:50%; overflow: hidden; position: relative; margin-right: 15px;`;
            ...
            btnplus.style.cssText = `width: 32px; height: 32px;z-index: 1; border-radius:50%; overflow: hidden; position: relative; margin-left: 15px;`;
            ...
            btnimg.style.cssText = `width: 30px; position: absolute; top: -1px; left: -2px;`
            btnhandle.appendChild(btnimg);
            btnhandle.style.cssText = `width: 30px; height: 30px; border-radius:50%;z-index: 1; overflow: hidden; position: relative;`
            ...
            btnminus.addEventListener('click', function(){
                ...
                let v =[]
                v = JSON.parse(window.localStorage.getItem('fastfood'));
        ...
        if (arrOrder.length) {
            ...
            shipping.style.cssText = `display: flex; justify-content: space-between; align-items: center; padding: 20px 0; font-size: 20px; border-bottom: 1px orange solid;`
            ...
            total.style.cssText = `display: flex; justify-content: space-between; font-size: 20px; padding: 20px 0; border-bottom: 1px orange solid; margin-bottom: 20px;`
            ...
            btn.style.cssText = `width: fitcontent; margin: auto; padding: 10px 40px; border: 2px solid orange; border-radius: 25px; font-size: 20px; font-weight: 600; `;
...
function updateData (quantity, ele){
    ...
    update(ref(db, `user/${username}/products/` + ele.id), {
        name: ele.name,
        id: ele.id,
        img: ele.img,
        price: ele.price,
...
function deleteData (ele){
    remove(ref(db, `user/${username}/products/` + ele.id)).catch(()=>{
        console.log(Error);
    });
}
function deleteAll (){
    remove(ref(db, `user/${username}/products`)).catch(()=>{
        console.log(Error);
    });
}