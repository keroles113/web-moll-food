...
let arrOrder = [];
// if (window.localStorage.getItem('order-fruits')) {
//     arrOrder = JSON.parse(window.localStorage.getItem('order-fruits'));
// }
...
fetch('https://login1form-3fad9-default-rtdb.firebaseio.com/CurrentUser.json').then((res)=>{
    ...
    fetch(`https://login1form-3fad9-default-rtdb.firebaseio.com/user/${username}/fruitProducts.json`).then((res)=>{
        ...
        arrOrder.forEach(ele => {
            ...
            btnminus.style.cssText = `width: 32px; height:z-index: 1; 32px; border-radius:50%; overflow: hidden; position: relative; margin-right: 15px;`;
        ...
        if (arrOrder.length) {
            ...
            shipping.style.cssText = `display: flex; justify-content: space-between; align-items: center; padding: 20px 0; font-size: 20px; border-bottom: 1px rgb(25, 171, 35) solid;`
            ...
            total.style.cssText = `display: flex; justify-content: space-between; font-size: 20px; padding: 20px 0; border-bottom: 1px rgb(25, 171, 35) solid; margin-bottom: 20px;`
            ...
            btn.style.cssText = `width: fitcontent; margin: auto; padding: 10px 40px; border: 2px solid rgb(35, 76, 36); border-radius: 25px; font-size: 20px; font-weight: 600; `;
            ...
            pay.forEach(element => {
                element.addEventListener('click' ,()=>{
                    if (element.classList.contains('receipt')) {
                        ...
                        // }else{
                        //     totalPric.innerHTML = (parseFloat(totalPric.innerHTML) + parseFloat(ships.innerHTML)).toFixed(2) + "$";
                        // }
...
function updateData (quantity, ele){
    update(ref(db, `user/${username}/fruitProducts/` + ele.id), {
        name: ele.name,
        id: ele.id,
        img: ele.img,
        price: ele.price,
...
function deleteData (ele){
    remove(ref(db, `user/${username}/fruitProducts/` + ele.id)).catch(()=>{
        console.log(Error);
    });
}
function deleteAll (){
    remove(ref(db, `user/${username}/fruitProducts/`)).catch(()=>{
        console.log(Error);
    });
}